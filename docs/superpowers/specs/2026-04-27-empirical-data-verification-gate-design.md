...
#### 도구 카테고리 추상화 원칙 (v1.1.1)

각 카테고리는 "검증 능력의 추상화"이며, MCP·CLI·Bash 명령은 **카테고리 안에서 등가의 fallback**이다. 환경에서 어느 하나라도 가용하면 그 카테고리는 `available`로 간주한다.

| 카테고리 | 가용 도구 (등가 fallback) |
|---------|--------------------------|
| `CODE_LOGIC` | Grep / Read / Glob, ripgrep, ag |
| `DB_STATE` | DB MCP (D1 등), Bash CLI: sqlcmd / psql / mysql / sqlite3 / mongosh 등 |
| `LOG_PATTERN` | `mcp__grafana__query_loki_logs`, 로컬 파일 Read+Grep, Bash: tail / awk / journalctl |
| `METRIC` | `mcp__grafana__query_prometheus`, Bash: curl로 Prometheus HTTP API, promtool query, export된 CSV |
| `PROFILE` | `mcp__grafana__query_pyroscope`, Bash: perf / pprof, flame graph dump |
| `DASHBOARD` | `mcp__grafana__search_dashboards` / `get_dashboard_by_uid`, dashboard JSON 파일 Read, Bash curl |
| `EXTERNAL_API` | WebFetch, Bash: curl / wget, 명시된 SDK CLI |

환경 가용성 점검은 다음 순서를 따른다:

1. **MCP 점검:** 도구 정의가 사용 가능한 도구 목록에 등재돼 있는지.
2. **CLI 점검 (MCP 미가용 시):** `command -v {cli명}` 1회 실행으로 PATH 존재 확인. 카테고리당 후보 CLI 중 하나라도 발견되면 충족.
3. **로컬 도구 점검:** Grep / Read / Glob은 항상 available.

판정: 카테고리의 어느 후보 도구라도 가용하면 `available`. 모두 미가용일 때만 `unavailable`로 `UNAVAILABLE_TOOLS`에 기록.

#### EVIDENCE_TOOLS_USED 기록 형식 (v1.1.1)

Advocate는 각 claim별로 실제 호출한 도구를 다음 형식으로 기록한다:

- **MCP 호출:** 도구명 그대로 (예: `mcp__grafana__query_prometheus`).
- **CLI / Bash 호출:** `Bash(<명령 요약>)` (예: `Bash(psql -c "SELECT ...")`, `Bash(curl http://prom/api/v1/query?...)`). 명령 요약은 1줄, 자격증명·토큰은 마스킹.
- **로컬 도구:** `Grep` / `Read` / `Glob` 그대로.
- 한 claim에 여러 도구 사용 시 쉼표로 join.

Tool Coverage Audit은 도구 종류와 무관하게 "REQUIRED_TOOLS 카테고리에 해당하는 도구를 어떤 형태로든 호출했는가"를 본다. CLI 호출도 동등하게 인정.
