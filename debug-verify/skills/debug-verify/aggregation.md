...
- `EVIDENCE_TOOLS_USED:` 아래의 Claim ID별 호출 도구 목록 (각 줄 `- {Claim ID}: {도구 목록 또는 'none'}`)
...
- `EVIDENCE_TOOLS_USED` 블록 누락 → Tool Coverage Audit에서 모든 claim을 도구 미호출로 간주 (보수적 처리).

## Step 1.5: Tool Coverage Audit

> **실행 순서:** Step 2 (Verdict Consensus) **이전**에 수행. CLAIM_TOOL_MAP이 있을 때만 적용.

1. Step 1.5에서 SKILL.md가 생성한 `CLAIM_TOOL_MAP`을 입력으로 받는다 (없으면 이 단계 스킵).
2. 각 claim에 대해 `REQUIRED_TOOLS`(CLAIM_TOOL_MAP) vs `EVIDENCE_TOOLS_USED`(Advocate 출력)를 대조한다.
3. REQUIRED_TOOLS 중 하나라도 EVIDENCE_TOOLS_USED에 없으면 — Advocate가 CONFIRMED를 내렸어도 — 해당 claim 판정을 **INCONCLUSIVE로 강제 변환**한다. CONFIDENCE는 LOW로 하향.
4. UNAVAILABLE_TOOLS 때문에 도구 호출이 불가능했던 claim은 별도 분류 (Advocate가 이미 INCONCLUSIVE 처리했어도 Tool Coverage 섹션에 명시).
5. 강제 변환 결과를 메모리 내 `coverage_audit` 객체에 기록:

```
coverage_audit = [
  { claim_id: "C1", original_verdict: "CONFIRMED", final_verdict: "INCONCLUSIVE",
    missing_tools: ["DB_STATE"], reason: "tool-not-called" },
  { claim_id: "C2", original_verdict: "INCONCLUSIVE", final_verdict: "INCONCLUSIVE",
    missing_tools: ["METRIC"], reason: "env-blocked" },
]
```

6. 강제 변환된 claim이 하나라도 있으면 전체 VERDICT 재계산:
   - 모든 핵심 claim이 INCONCLUSIVE로 변환되면 전체 VERDICT를 INCONCLUSIVE로 강등.
   - 일부 claim만 변환되면 핵심 claim 판정 룰(advocate-context.md §4)을 다시 적용.
7. `coverage_audit`은 Step 4 Action 결정과 Final Report의 `Tool Coverage` 섹션에 사용된다.
...
## Tool Coverage Section (Final Report 추가 영역)

`coverage_audit`이 비어있지 않으면 Final Report에 다음 섹션을 추가한다:

```
### Tool Coverage
- {Claim ID}: {원판정} → {최종판정} (missing: {빠진 도구 목록}) [{tool-not-called|env-blocked}]
- ...
```

`tool-not-called`는 Advocate가 호출을 누락한 경우, `env-blocked`는 UNAVAILABLE_TOOLS 때문에 호출 자체가 차단된 경우다. env-blocked 항목은 Advocate 책임이 아니므로 사용자에게 환경 보강을 안내한다.
