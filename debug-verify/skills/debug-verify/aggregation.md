...
### 하위호환:
- Advocate가 구 `NEXT_ACTIONS:` 블록만 반환하면 `FIX_CANDIDATES:` 없음으로 처리. CONFIRMED 판정 시 Step 5.5(SKILL.md)는 스킵되고 기존 "수정 계획 작성" 안내만 출력.
...
|-----------------------|------|
| **≥ 2** | SKILL.md **Step 5.5 "Fix Direction Selection"**으로 분기하여 AskUserQuestion으로 사용자 선택. 선택 결과를 `selected_fix_direction`에 저장하고 Step 6(Final Report)의 "Selected Fix Direction" 필드에 주입. |
| **== 1** | 단일 `[recommended]` 후보를 그대로 "Selected Fix Direction"에 주입. AskUserQuestion 생략. |
